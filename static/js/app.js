...
function renderRows(lista) {
    ...
    if (!tbody) return;
    if (summary) {
        summary.textContent = `${lista.length} chip(s) exibido(s) de ${ALL_CHIPS.length}.`;
    }
...
function atualizarEstadoSlot() {
    const aparelhoEl = document.getElementById("modal_sk_aparelho_atual");
    const slotEl = document.getElementById("modal_slot_whatsapp");
    if (!aparelhoEl || !slotEl) return;

    const temAparelho = Boolean(aparelhoEl.value);
    slotEl.disabled = !temAparelho;
    if (!temAparelho) slotEl.value = "";
}

document.getElementById("modal_sk_aparelho_atual")?.addEventListener("change", atualizarEstadoSlot);
...
document.getElementById("modalSaveBtn")?.addEventListener("click", async () => {
    ...
    if (!data.sk_aparelho_atual) data.slot_whatsapp = null;
